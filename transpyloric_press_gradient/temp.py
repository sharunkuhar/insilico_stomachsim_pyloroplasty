...
# Proof that value at t=5 is approximately the average value
plt.style.use('halfcol')
fig, ax = plt.subplots()
ax.plot(t_hlty, dp_hlty, label='1 mPa s')
ax.plot([0,20], [dpbyL_hlty*l_pressure, dpbyL_hlty*l_pressure], 'k--')
ax.plot(t_re2hlty, dp_re2hlty, label='50 mPa s')
ax.plot([0,20], [dpbyL_re2hlty*l_pressure, dpbyL_re2hlty*l_pressure], 'k--')
ax.plot([2.5,2.5],[0,0.6], 'k-')
ax.plot([5,5],[0,0.6], 'k-')
ax.set_xlim(0, 20)
ax.set_ylabel(r'$p_A - p_D$ (mm Hg)')
ax.set_xlabel('t (sec)')
ax.legend()
ax.set_title('value at t=5 ~ avg value')
